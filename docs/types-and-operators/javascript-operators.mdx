...
重點補充說明，我們前面說過 `b = c` 這種運算子的本質是一個函式，而等號運算子的函式會把左邊的值「設定成右邊的值」，然後「回傳右邊的參數」。
也就是 b 會被設定成 c 的值，並且回傳 c，接下去以此類推，最後 a、b、c 就會都等於 3 了。
...
> 到這裡，我們已經知道運算子就是函式，因此強制型別轉換是呼叫函式的一部分，也是動態型別的一部分。
...
我們會得到 NaN，表示「不是數字 (Not a Number)」的意思，它不完全是純值，但可以當作它是。
NaN 代表有個東西想要轉換成數值型別，但它不是數字，所以無法轉換。

但是如果是 null 的話，JavaScript 則是會將 null 當作 0 的意思。

```javascript
Number(null); // 0
```

所以不是每個強制型轉都能明顯判斷，有些需要記起來。
...
在開發時，如果不是刻意需要型轉，都建議使用[三等號](https://sealman234.github.io/2019-08-03-javascript-equal-operator/)的比較運算子，只要資料型別不同，回傳結果就是 false！
...
呼叫函式後，首先會創造一個新的執行環境 `greet()` 與變數 `name`，並且在記憶體中設定變數 `name` 為預設值 `undefined`。
...
如果我們不想用 `undefined` 當參數的預設值，可以使用「或」運算子來設定預設值，它會回傳第一個被轉換成 true 的參數。
換句話說，如果左邊的參數被轉換成 Boolean 後是 false，就會回傳右邊的參數。
...
現在我們知道在瀏覽器的全域物件 `window` 下有 `libraryName` 這個全域變數，我們可以結合「或」運算子的用法，來檢查全域命名空間 (Global Namespace) 或全域物件裡面是否已經有相同的名稱。
