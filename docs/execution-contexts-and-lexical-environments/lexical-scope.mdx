---
sidebar_position: 2
keywords: [javascript, Lexical, scope]
---

# 語法作用域

JavaScript 是屬於語法作用域（靜態作用域），而靜態作用域與動態作用域兩者有何不同呢。
...
下方範例中，因為 JavaScript 的作用域是在**函式**裡面，因此函式內會讀到 `Sealman` 這個變數，但是外層是讀不到的。
...
callName(); // 海豹人
console.log(Sealman); // Sealman is not defined
```
