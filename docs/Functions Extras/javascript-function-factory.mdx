...
例如：另外創造兩個函式，再將特定的參數值 `en` 和 `es` 分別先帶入函式的內容中。
完成後，如果要用英文打招呼就呼叫 `greetEnglish` 函式，用西班牙語則呼叫 `greetSpanish` 函式，這樣在使用時就不用考慮 `language` 這個參數了。
...
當內部函式被呼叫時，`language` 會被包在閉包裡面，它是一個[自由變數](https://sealman234.github.io/2021-04-01-javascript-closure/)，內部函式隨著範圍鏈往外找就可以參考到它。
